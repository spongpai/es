...
$(function() {
    ...
    function populateDSList(){
           ...
           //            '179', '77', '73', '181', '74',
           //            '180', '75'];
           var names = ["trash_sensors:fill","micro_reports:fill","events:fill","agg","events:fill2","query:agg","query:group"];
           var values = ['6','7','8','9', '10', "q1","q2"];
    ...
    var mapOptions = {
        ...
        //center: {lat: 33.5, lng: -117.0},   // uci
        //zoom: 8,
        center: {lat: 38.9048, lng: -77.0163},   // dc 	-77.0163, 38.9048
        zoom: 10,
    ...
    function addEventOnMarkers(){
        ...
        for(var i = 0; i < map_cells.length; i++){
            ...
            google.maps.event.addListener(marker, 'mouseover', function(event) {
                infowindow.setContent("<p>value: " + this.value+ "</p>");
    ...
    function setTimeLine(){
        ...
        timeline.on('select', function (properties) {
            ...
            //formData["dsname"] = 5;
            //var latlngFormat = "array"; // array
            var latlngFormat = "latlon";
            //alert('dsid:' + formData["dsname"] + 'selected items: ' + sDate.getTime() + ', ' + eDate.getTime());

            var strDS = formData["dsname"];
            var type = 'emage';
            if(strDS.indexOf('q') > -1){
                type = 'query';
            }
            var query = 'rest/sttwebservice/search/'+formData["dsname"]+'/' + latlngFormat
            +'/'+formData["swLat"]+','+formData["swLng"] + '/'+formData["neLat"]+','+formData["neLng"]
            +'/'+sDate.getTime()+'/'+ eDate.getTime() + '/' + getLatLonRes() + '/'+formData["spagg"]+'/fill/' + type;
